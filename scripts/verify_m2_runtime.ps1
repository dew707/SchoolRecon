...
if ($DotnetVer -notmatch "^8\.") {
    ...
    if ($sdks -notmatch "^8\.") {
        Write-Host "FAILED" -ForegroundColor Red
        Write-Host "BLOCKED: DOTNET_NOT_AVAILABLE" -ForegroundColor Red
        Write-Host "Explanation: .NET 8 SDK is required. Found: $DotnetVer."
        exit 1
    }
}
...
try {
    dotnet restore SchoolRecon.sln | Out-Null
    Write-Host "PASS" -ForegroundColor Green
} catch {
...
try {
    dotnet build SchoolRecon.sln --no-restore -c Release | Out-Null
    Write-Host "PASS" -ForegroundColor Green
} catch {
    Write-Host "FAILED: DOTNET_BUILD_FAILED" -ForegroundColor Red
    Pop-Location
...
# STEP 11: Start real ASP.NET Core API
...
$ApiProcess = Start-Process dotnet -ArgumentList "run --project `"$RepoRoot\backend\SchoolRecon.Api`" --no-build -c Release" -PassThru -NoNewWindow
...
Push-Location "$RepoRoot"
python3 tests/test_agent_regression.py | Out-Null
if ($LASTEXITCODE -ne 0) {
