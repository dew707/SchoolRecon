...
| **Navigation Workflow Configuration** | `LIVE_VERIFIED` | Ordered navigation steps load and save through React/reconService/API/application/Dapper/stored procedures/SQL Server. Add, edit, duplicate, reorder, enable/disable, delete, validation, atomic rollback, independent SQL verification, and API-restart persistence passed. Credential steps persist semantic field references only. |
...
- **Navigation Workflow Runtime Accepted:** The real Docker SQL Server and ASP.NET/Dapper path passed P01-M2-T04 workflow persistence, deterministic resequencing, fail-closed validation, atomic rollback, restart persistence, and independent SQL verification.
