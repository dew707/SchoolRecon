# SchoolRecon â€” Milestone 2 (M2) Runtime Acceptance Guide
...
SQLSERVER_USER=sa
SQLSERVER_PASSWORD=<SET_EXTERNALLY>
...
```bash
docker run -e "ACCEPT_EULA=Y" -e "MSSQL_SA_PASSWORD=<SET_EXTERNALLY>" \
...
sqlcmd -S localhost,1433 -U sa -P "<SET_EXTERNALLY>" -Q "IF DB_ID('SchoolRecon') IS NULL CREATE DATABASE [SchoolRecon];"
sqlcmd -S localhost,1433 -U sa -P "<SET_EXTERNALLY>" -d SchoolRecon -i deploy_all.sql
cd ../..
```
